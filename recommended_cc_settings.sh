...
if ((BASH_VERSINFO[0] < 4)); then
    ...
    echo "On macOS: brew install bash, then run with: /opt/homebrew/bin/bash $0"
    exit 1
fi
...
prompt_misc_guards() {
    ...
    print_section_header "$section_num" "Miscellaneous Safety Guards" "${TOTAL_SECTIONS}"
    echo ""
